...
<div class="room1-content">
	...
	<div class="room1-title-detail">
	    <div class="toggle-container">
	        ...
	        <span class="toggle-text">설명 보기 ${result}</span>
	    </div>
	...
	<div style="margin-bottom: 50px;">
		<textarea id="ideaDetailInput" class="room1-subject" style="height: 150px; white-space: pre-wrap;"
...
function submitIdeaForm1() {
    ...
    var originalDetail = "${submittedIdea.getDescription()}".replace(/\\n/g, '\n');
    ...
    document.getElementById('ideaDetailHidden').value = ideaDetail.replace(/\n/g, '\\n');
...
function updateForm() {
    ...
    document.getElementById('ideaDetailHidden2').value = ideaDetail.replace(/\n/g, ' ');
...
function openAiLogModal() {
    document.getElementById("aiLogModal").style.display = "block";
    loadAiLog();
}
...
function adjustResponseHeight() {
    ...
    const availableHeight = containerHeight - otherQueryHeight - buttonsHeight - 40;
    
    wrapper.style.height = `${availableHeight}px`;
    
    const contentHeight = text.offsetHeight + otherQueryHeight + buttonsHeight + 40;
    if (contentHeight > containerHeight) {
        container.style.height = `${Math.min(contentHeight, 550)}px`;
    }
    ...
    .then(data => {
        ...
        if (responseText && responseContainer && responseWrapper) {
            if (jsonData.error) {
                ...
                console.log("AI response:", jsonData.aiResponse);
